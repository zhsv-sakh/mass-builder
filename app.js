...
function drawLine(canvas, series, color, labels, selectedIdx){
  ...
  let ticks = Math.round((niceMax - niceMin) / step);

  while(ticks > targetTicks + 1){
    step = step * 2;
    niceMin = Math.floor(min / step) * step;
    niceMax = Math.ceil(max / step) * step;
    ticks = Math.round((niceMax - niceMin) / step);
  }
  while(ticks < 3 && step > 0.001){
    step = step / 2;
    niceMin = Math.floor(min / step) * step;
    niceMax = Math.ceil(max / step) * step;
    ticks = Math.round((niceMax - niceMin) / step);
  ...
  const yMin = niceMin, yMax = niceMax;
  const yAt = v => pad.t + ch * (1 - (v - yMin) / (yMax - yMin));

  const decimals = isPoints ? 1 : (Math.abs(step - Math.round(step)) < 0.0001 ? 0 : 1);
